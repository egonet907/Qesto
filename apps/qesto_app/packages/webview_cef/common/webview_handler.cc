...
bool WebviewHandler::OnBeforePopup(CefRefPtr<CefBrowser> browser,
    ...
    if (isAllowedTopLevelUrl(browser->GetIdentifier(), target_url.ToString())) {
...
void WebviewHandler::OnLoadError(CefRefPtr<CefBrowser> browser,
    ...
    // Never reflect a banking URL (which may contain sensitive query data)
    // into a data: page or production log.
}
...
bool WebviewHandler::OnBeforeBrowse(CefRefPtr<CefBrowser> browser,
    ...
    return !isAllowedTopLevelUrl(browser->GetIdentifier(),
...
bool WebviewHandler::OnOpenURLFromTab(CefRefPtr<CefBrowser> browser,
                                      ...
                                      bool user_gesture) {
    if (isAllowedTopLevelUrl(browser->GetIdentifier(), target_url.ToString())) {
        loadUrl(browser->GetIdentifier(), target_url.ToString());
    }
    return true;
}

bool WebviewHandler::OnCertificateError(CefRefPtr<CefBrowser> browser,
                                        ErrorCode cert_error,
                                        const CefString& request_url,
                                        CefRefPtr<CefSSLInfo> ssl_info,
                                        CefRefPtr<CefCallback> callback) {
    if (onCertificateErrorEvent) {
        onCertificateErrorEvent(
            browser->GetIdentifier(), normalizedOrigin(request_url.ToString()));
    }
    return false;
}
...
bool WebviewHandler::CanDownload(CefRefPtr<CefBrowser> browser,
                                 const CefString& url,
                                 const CefString& request_method) {
    return false;
}

bool WebviewHandler::OnPreKeyEvent(CefRefPtr<CefBrowser> browser,
    ...
    if (key == VK_F6 || key == VK_F12) return true;
    if (ctrl && (key == 'L' || key == 'T' || key == 'N' || key == 'O')) return true;
    if (ctrl && shift && (key == 'N' || key == 'T')) return true;
    return false;
}
...
void WebviewHandler::createBrowser(std::string url,
                                   ...
                                   std::function<void(int)> callback)
{
    ...
    if(!CefCurrentlyOn(TID_UI)) {
		CefPostTask(TID_UI, base::BindOnce(&WebviewHandler::createBrowser, this,
            url, profile_path, allowed_origins, devtools_enabled, callback));
		return;
	}
#endif
    ...
    for (const auto& origin : allowed_origins) {
        ...
        if (!normalized.empty()) security.allowed_origins.insert(normalized);
    }

    CefRequestContextSettings context_settings;
    CefString(&context_settings.cache_path) = profile_path;
    ...
    // 60 FPS avoids a separate begin-frame pump while retaining accelerated
    // OnAcceleratedPaint rendering.
    ...
    // Do not start an external begin-frame driver before CefInitialize. Doing
...
bool WebviewHandler::StartDragging(CefRefPtr<CefBrowser> browser,
                                  ...
                                  int x,
                                  int y){
    // Prevent URL and local-file drag operations from escaping or entering the
    // controlled banking surface.
    return true;
}
...
{
    auto it = browser_map_.find(browserId);
    if (it != browser_map_.end() && isAllowedTopLevelUrl(browserId, url)) {
...
void WebviewHandler::stopLoading(int browserId) {
    auto it = browser_map_.find(browserId);
    if (it != browser_map_.end()) it->second.browser->StopLoad();
}

bool WebviewHandler::canGoBack(int browserId) {
    auto it = browser_map_.find(browserId);
    return it != browser_map_.end() && it->second.browser->CanGoBack();
}

bool WebviewHandler::canGoForward(int browserId) {
    auto it = browser_map_.find(browserId);
    return it != browser_map_.end() && it->second.browser->CanGoForward();
}

void WebviewHandler::openDevTools(int browserId) {
    auto it = browser_map_.find(browserId);
    auto security = security_map_.find(browserId);
    if (it != browser_map_.end() && security != security_map_.end() &&
        security->second.devtools_enabled) {
        CefWindowInfo windowInfo;
#ifdef OS_WIN
...
std::string WebviewHandler::normalizedOrigin(const std::string& url) {
    CefURLParts parts;
    ...
    const auto lowerAscii = [](char value) {
        return static_cast<char>(std::tolower(static_cast<unsigned char>(value)));
    };
    std::transform(scheme.begin(), scheme.end(), scheme.begin(), lowerAscii);
    std::transform(host.begin(), host.end(), host.begin(), lowerAscii);
    if (scheme != "https" || host.empty()) return "";
    std::string origin = scheme + "://" + host;
    if (!port.empty() && port != "443") origin += ":" + port;
    return origin;
}

bool WebviewHandler::isAllowedTopLevelUrl(int browserId,
                                          const std::string& url) const {
    if (url == "about:blank") return true;
    auto config = security_map_.find(browserId);
    if (config == security_map_.end()) return false;
    const std::string origin = normalizedOrigin(url);
    return !origin.empty() &&
           config->second.allowed_origins.find(origin) !=
               config->second.allowed_origins.end();
}
...
void WebviewHandler::setJavaScriptChannels(int browserId, const std::vector<std::string> channels)
{
    ...
    for(auto& channel : channels)
    {
        ...
        extensionCode += " = (e,r) => {qestoExternal.JavaScriptChannel('";
        extensionCode += channel;
        extensionCode += "',e,r)};";
    }
        ...
        if(bit != browser_map_.end() && bit->second.browser.get()){
            ...
            if (frame)
            {
                ...
                if(callback != nullptr){
                    ...
                    finalCode = "qestoExternal.EvaluateCallback('";
