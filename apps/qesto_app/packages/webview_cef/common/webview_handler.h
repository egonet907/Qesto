...
struct browser_security_config {
...
public:
    ...
    std::function<void(int browserId, std::string origin)> onCertificateErrorEvent;
    ...
    // CefRequestHandler methods. Only top-level navigation is restricted;
    // HTTPS subresources required by the bank continue to use Chromium's
    // normal network stack and security model.
    ...
    // CefContextMenuHandler / CefDownloadHandler / CefKeyboardHandler.
...
private:
    bool isAllowedTopLevelUrl(int browserId, const std::string& url) const;
    static std::string normalizedOrigin(const std::string& url);
