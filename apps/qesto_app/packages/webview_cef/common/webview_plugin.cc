...
namespace webview_cef {
	...
	void WebviewPlugin::initCallback() {
		if (!m_init)
		{
			...
			// Production banking pages are not mirrored into Dart/stdout. Console
			// content can contain account data or authorization diagnostics.
			...
			m_handler->onCertificateErrorEvent = [=, this](int browserId, std::string origin) {
				if (m_invokeFunc) {
					WValue* bId = webview_value_new_int(browserId);
					WValue* wOrigin = webview_value_new_string(origin.c_str());
					WValue* retMap = webview_value_new_map();
					webview_value_set_string(retMap, "browserId", bId);
					webview_value_set_string(retMap, "origin", wOrigin);
					m_invokeFunc("onCertificateError", retMap);
					webview_value_unref(bId);
					webview_value_unref(wOrigin);
					webview_value_unref(retMap);
				}
			};
	...
	void WebviewPlugin::uninitCallback(){
		...
		m_handler->onCertificateErrorEvent = nullptr;
		m_init = false;
	}
    ...
    void WebviewPlugin::HandleMethodCall(std::string name, WValue* values, std::function<void(int ,WValue*)> result) {
		if (name.compare("init") == 0){
			...
			std::string profilePath = webview_value_get_string(webview_value_get_list_value(values, 1));
			WValue* originsValue = webview_value_get_list_value(values, 2);
			std::vector<std::string> origins;
			for (size_t i = 0; i < webview_value_get_len(originsValue); ++i) {
				origins.emplace_back(webview_value_get_string(webview_value_get_list_value(originsValue, i)));
			}
			bool devtoolsEnabled = webview_value_get_bool(webview_value_get_list_value(values, 3));
			m_handler->createBrowser(url, profilePath, origins, devtoolsEnabled,
			[=, this](int browserId) {
				if (browserId < 0) {
					result(0, nullptr);
					return;
			...
			result(1, nullptr);
				 ...
				 name.compare("canGoForward") == 0) {
			int browserId = int(webview_value_get_int(values));
			bool value = name.compare("canGoBack") == 0
				? m_handler->canGoBack(browserId)
				: m_handler->canGoForward(browserId);
			WValue* response = webview_value_new_bool(value);
			result(1, response);
			webview_value_unref(response);
		}
	...
	int initCEFProcesses(CefMainArgs args, void* sandbox_info)
	{
		mainArgs = args;
		cefSandboxInfo = sandbox_info;
...
#endif
		return CefExecuteProcess(mainArgs, app, cefSandboxInfo);
	}
...
#endif
		isCefInitialized = CefInitialize(mainArgs, cefs, app.get(), cefSandboxInfo);
	}
